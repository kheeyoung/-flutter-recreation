...
class NotificationController extends GetxController {
  ...
  void _onMessage() async {
    ...
    await flutterLocalNotificationsPlugin.initialize(
      ...
      //onDidReceiveNotificationResponse: () {} 이거 알림 터치시 행동인데 일단 보류. (앱이 실행되게 하면 좋을 것 같음. 단, 로그인 여부 따라서 달라야함
    );
  ...
  Future<void> sendNotification(
    ...
    try {
      ...
      if (response.statusCode == 200) {
        print("알림 전송 성공: ${response.body}");
      } else {
        print("알림 전송 실패: ${response.statusCode} - ${response.body}");
      }
    } catch (e) {
      print("오류 발생: $e");
    }
