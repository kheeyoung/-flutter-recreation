    ...
    return GestureDetector(
      ...
      child: Scaffold(
        ...
        body: SingleChildScrollView(
          child: Center(
            child: FutureBuilder(
              future: Future.wait([usermethod.getUserName(), itemmethod.getMyItem(user!.uid)]),
                ...
                if (snapshot.hasData) {
                  ...
                  return Column(
                    children: [
                        ...
                        children: [
                          ...
                          Container(
                            ...
                            child: TextFormField(
                              ...
                              decoration: inputTextFormField.basicFormDeco("제목을 입력해주세요."),
                            ),
                          ),
                        ],
                      ),
                        ...
                        children: [
                          ...
                          Container(
                            ...
                            child: TextFormField(
                              ...
                              decoration: inputTextFormField.basicFormDeco("내용을 입력해주세요."),
                            ),
                          ),
                        ],
                      ),
                      ...
                      OutlinedButton(
                          onPressed: () async{
                            if(title.isNotEmpty && contents.isNotEmpty && SelectedGift != "" && SelectedRecipient != ""){
                              //board DB에 글 등록
                              String recipientUid = await usermethod.getUserUidByName(SelectedRecipient);


                              await giftmethod.SendGift(SelectedGift, user.uid, recipientUid, title, contents, SelectedRecipient);

                              //아이템 사용 전환 (가장 오래 된 것 부터 사용)
                              int result = await giftmethod.useItem(SelectedGift, user.uid);

                              if (result != 0){ //만약 없는 아이템을 사용한 경우
                                myNotification.SnackbarBasic(context, "오류! 새로고침 후 다시 시도해주세요. 오류가 계속 될 경우 총괄계 제보 바랍니다.");

                              }
                              else{ //잘 보내진 경우
                                //호감도 체크
                                if(recipientUid==snapshot.data![1][SelectedGift]){
                                  giftmethod.addLikePoint(user.uid,recipientUid,SelectedGift);
                                }
                                myNotification.SnackbarBasic(context, "선물 발송 완료!!");
                              ...
                              myNotification.SnackbarBasic(context, "제목, 수신인, 선물, 내용은 필수 항목 입니다.");
